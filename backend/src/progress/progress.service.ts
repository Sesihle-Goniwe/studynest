...
export class ProgressService {
  ...
  async findAllTopicsForUser(userId: string) {
    ...
    if (error) {
      ...
      throw new InternalServerErrorException('Failed to fetch topics.', error.message);
    }
    return data;
  }
  // Log study hours
async addStudyLog(createStudyLogDto: CreateStudyLogDto) {
  ...
  if (error) {
    ...
    throw new InternalServerErrorException('Failed to add study log.', error.message);
  }
  return data;
}
...
async getStudyLogs(userId: string, topicId?: string) {
  ...
  if (error) {
    ...
    throw new InternalServerErrorException('Failed to fetch study logs.', error.message);
  }
...
async updateStatus(id: string, status: string) {
  ...
  if (error) {
    throw new InternalServerErrorException('Failed to update topic status.', error.message);
  }
  return data;
}
  ...
  if (logError) {
    throw new InternalServerErrorException('Failed to delete associated study logs.', logError.message);
  }
  ...
  if (topicError) {
    throw new InternalServerErrorException('Failed to delete topic.', topicError.message);
  }
