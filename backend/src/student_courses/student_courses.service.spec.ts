...
describe('StudentCoursesService', () => {
      ...
      data: [
        {
          ...
          students: { user_id: 'peerA', university: 'Wits', year: 2, profileImage: 'a.png' },
        },
        {
          student_id: 'peerA',
        ...
        },
        {
          student_id: 'peerB',
          courses: { id: 'c1', course_code: 'MATH' },
          students: { user_id: 'peerB', university: 'UCT', year: 3, profileImage: 'b.png' },
        },
      ],
      error: null,
    });
  ...
  it('findMatchingStudents: returns [] if user has no courses', async () => {
    tableMap.student_courses.eq.mockResolvedValueOnce({ data: [], error: null });
  ...
  it('addCourse: inserts a brand new course', async () => {
    tableMap.courses.single
      ...
      .mockResolvedValueOnce({ data: { id: 'new-id', course_code: 'ACC1', course_name: 'Acct' }, error: null }); // after insert

    tableMap.courses.insert.mockReturnValue(tableMap.courses);
    tableMap.courses.select.mockReturnValue(tableMap.courses);

    const dto = await service.addCourse({ course_code: 'ACC1', course_name: 'Acct' });
    expect(dto.id).toBe('new-id');
  });

  it('addCourse: throws if already exists', async () => {
    tableMap.courses.single.mockResolvedValueOnce({ data: { id: 'c1' }, error: null }); // existing
    await expect(service.addCourse({ course_code: 'ACC1', course_name: 'Acct' }))
  ...
  it('addMatch: upserts liked', async () => {
    ...
    tableMap.matched_students.select.mockResolvedValueOnce({ data: [{ id: 1 }], error: null });
  ...
  it('updateMatchStatus: updates and returns rows', async () => {
    ...
    tableMap.matched_students.eq.mockResolvedValueOnce({ data: [{ id: 1, status: 'rejected' }], error: null });
  ...
  it('getMyMatches: filters out self-matches', async () => {
    ...
    tableMap.matched_students.eq.mockResolvedValueOnce({
      data: [
        { id: 1, status: 'liked', matched_user_id: 'peer1', students: { user_id: 'peer1', university: 'Wits', year: 2 } },
        { id: 2, status: 'matched', matched_user_id: 'u1', students: { user_id: 'u1', university: 'Wits', year: 3 } }, // self
      ],
      error: null,
    });
