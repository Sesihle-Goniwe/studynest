...
describe('StudentsService', () => {
  ...
  beforeEach(async () => {
    ...
    supabaseMock = {
      getClient: jest.fn().mockReturnValue({
        ...
        single: jest.fn().mockResolvedValue({ data: { user_id: '123', name: 'Test' }, error: null }),
        storage: {
          from: jest.fn().mockReturnThis(),
          upload: jest.fn().mockResolvedValue({ data: {}, error: null }),
          getPublicUrl: jest.fn().mockReturnValue({ data: { publicUrl: 'http://mock-url.com/avatar.png' } }),
        },
      }),
    };
  ...
  it('should get all students', async () => {
    ...
    supabaseMock.getClient().from().select.mockResolvedValue({ data: mockData, error: null });
  ...
  it('should upload profile image and update student', async () => {
    ...
    const result = await service.updateStudentWithImage('123', { name: 'Updated' }, file);
    expect(result).toEqual({ user_id: '123', name: 'Test' });
  });
});
