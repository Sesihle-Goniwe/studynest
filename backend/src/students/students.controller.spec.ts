...
describe('StudentsController', () => {
  ...
  const mockStudentsService = {
    ...
    updateStudentP: jest.fn().mockResolvedValue({ uid: '123', name: 'John Updated' }),
    updateStudentWithImage: jest.fn().mockResolvedValue({ uid: '123', name: 'John', photoUrl: 'photo.png' }),
  };
  ...
  describe('updatePhoto', () => {
    it('should update student photo', async () => {
      ...
      expect(result).toEqual({ uid: '123', name: 'John', photoUrl: 'photo.png' });
      expect(service.updateStudentWithImage).toHaveBeenCalledWith('123', {}, mockFile);
    });

    it('should throw error if no file uploaded', async () => {
      await expect(controller.updatePhoto('123', undefined)).rejects.toThrow('No file uploaded');
    });
  });
});
